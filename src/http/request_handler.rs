...
pub struct RequestHandler {
    ...
    id_pool: Arc<Mutex<UserSessionPool>>,
    config: Config,
}

impl RequestHandler {

    pub fn new( page_pool: Arc<Mutex<PagePool>>, id_pool: Arc<Mutex<UserSessionPool>>,
            config: Config) -> RequestHandler {
        RequestHandler{ page_pool: page_pool, id_pool: id_pool, config: config }
    }
